...
export function RoiCalculator({ slug }: RoiCalculatorProps) {
  ...
  const cfg = roi.roiBySlug[slug];
  const [hoursSpent, setHoursSpent] = useState(10);
  const [hourlyRate, setHourlyRate] = useState(1500);
  const weeklySavingsHours = Math.round(hoursSpent * cfg.ratio);
  const monthlySavingsMoney = Math.round(weeklySavingsHours * 4 * hourlyRate);
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.4 }} className="w-full space-y-6 rounded-xl border border-white/5 bg-[#0d0d11] p-6">
      <div>
        <span className="rounded-md border border-[#00d2ff]/10 bg-[#00d2ff]/5 px-2 py-0.5 font-mono text-[10px] uppercase tracking-widest text-[#00d2ff]">{roi.badge}</span>
        <h4 className="mt-3 text-lg font-bold tracking-tight text-white">{roi.title}</h4>
        <p className="mt-1 text-xs text-gray-500">{cfg.factorLabel}</p>
      </div>
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div className="space-y-4">
          <div className="space-y-2">
            <div className="flex justify-between font-mono text-xs"><span className="text-gray-400">{cfg.hoursLabel}</span><span className="font-bold text-white">{hoursSpent} {roi.hoursUnit}</span></div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between font-mono text-xs"><span className="text-gray-400">{roi.rateLabel}</span><span className="font-bold text-white">{hourlyRate} {roi.currency}</span></div>
        ...
        <div className="flex flex-col justify-center space-y-3 rounded-xl border border-white/[0.02] bg-black/30 p-4">
          <div className="text-xs text-gray-400">{cfg.resultHoursLabel}</div>
          ...
          <div className="border-t border-white/[0.03] pt-2 text-xs text-gray-400">{cfg.resultMoneyLabel}</div>
